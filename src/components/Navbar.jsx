        ...
        <nav className="z-10 flex justify-between items-center w-full h-20 text-white fixed bg-black px-4">
            ...
            <ul className="hidden md:flex">
                {links.map(({ id, link, href }) => (
                    <li key={id} className="px-4 cursor-pointer capitalize font-medium text-gray-400 hover:scale-110 duration-200">{id !== 6 ? <Link to={link} smooth duration={500} onClick={() => setNav(false)}>{link}</Link> : <a href={href}>{link}</a>}</li>
                ))}
            </ul>
